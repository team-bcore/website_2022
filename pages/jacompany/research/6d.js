        ...
        <div className="">
            ...
            <div className="px-4 py-24 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-40">
                <div className="max-w-screen-sm sm:mx-auto">
                    ...
                    <p className="font-ud text-gray-700 text-base">
                        建物内部の車両や人の位置測位を画像処理技術で実施する場合、管理対象となる車両や人にマーカーを貼り付け、施設内に設置した多数のカメラでマーカーを読取り運用するモデルが採択されてきた。しかしながら、床面積の広い大規模空間での運用では、カメラ台数が非常に多くなりマーカーの設置場所にも苦慮するという事情があり、小規模なエリアでしか画像処理方式での位置測位は行われてこなかった。<br></br>
                    ...
                    <p className="mt-4 mb-2 font-noto text-gray-900 text-base">
                        1-1）屋内大空間での位置測位（IPS Indoor Positioning System） の必要性
                    </p>
                    <p className="font-ud text-gray-700 text-base">
                        屋内位置測位 （IPS）とは、GPSを使用することができない屋内空間において、建物内部のヒトや車両、モノの位置情報を測定、推定する技術である。工場や物流センター等の、床面積が10,000㎡を超えるような屋内大空間内部の位置情報を正確に知りたいというニーズは大きく、今後の生産性向上や経営合理化に必須の技術と考えられている。<br></br>
                    ...
                    <p className="font-ud text-gray-700 text-base">
                        従来の屋内大空間 (倉庫、工場等) における位置測位は、複数の基地からの電波強度をもとに自己位置を測位する技術（RFID・ビーコン等）により行われることが一般的である。しかし、電波強度は室内にある金属の影響で反射・干渉・回折などにより大きく変動することがあるため、数センチ単位の誤差が要求される精度の高い測位を実現することは困難を極める。電波は人間の目では見ることができないため、キャリブレーション作業は複雑で、機器の調整には熟練度が必要とされる。また、トラブルの際には原因がどこにあるのか特定しづらい。<br></br>
                    ...
                    <p className="font-ud text-gray-700 text-base">
                        カメラの姿勢計算とは、大きさや形が既知の物体をカメラで撮影したとき、カメラでの見え方から既知の物体までの3次元的な距離（XYZ3軸）や向き（PTR3軸）を求める計算のことを指す。姿勢計算の一般的な使用例の一つとして、正方形のARマーカーの上に、3Dモデルをマーカーの上にいるようにディスプレイ上で表現する技術がある。ARマーカーは正方形であるという既知の情報を元にARマーカー上に3Dモデルを表示するだけなので実空間上の座標や大きさが分からなくても姿勢計算は成立する。<br></br>
                    ...
                    <p className="mt-4 mb-2 font-noto text-gray-900 text-base">1-4）カラービット利用の優位性</p>
                    <p className="font-ud text-gray-700 text-base">
                        ...
                        <br></br>複数一括読取りに適しており、画面内にあるカラービットは一定の大きさより大きければすべて同時に認識できる特徴を持つ。タグ形状の歪みにも強い特徴を持ち、広角カメラでの画面端で樽型や糸巻き型に歪んでいる場合でも均質に読み取りすることができる。同様にローリングシャッターによる画像の歪みにも対応できる。このような特徴により、バーコード、QRコードなどの二次元コード、RFIDやビーコンでは適用が難しかった場面でも有効な技術である。
                        <br></br>二次元コードやARマーカーで高精度な自己位置推定を行なう場合、1-3に記述したようにコード自体の形状を極めて大きくする必要がある。二次元コードやARマーカーは原則として正方形なので、実際に大きなタグを実空間上に多数貼り付けすることは実効性が乏しい。一方でカラービットは、ひも状の形状認識が可能という特性上、直線形状のコードは狭小エリアにも設置できるという長所がある。細長い形状であれば、工場や倉庫内の柱や梁などに貼り付けることができるようになるため、面的な専有領域が大きいARマーカーや二次元コードと比較すると設置難易度は大きく下がる。タグ製作コストも面積が小さくなるためより安価になるというメリットがある。
                        <br></br>これらの観点から、カメラの姿勢計算を用いて自己位置推定を行う場合に利用するコードとしてカラービットは極めて適しているといえる。
                    </p>
                    <p className="mt-4 mb-2 font-noto text-gray-900 text-base">
                    ...
                    <p className="font-ud text-gray-700 text-base">
                        ・ XYZの座標系は右手座標系で、屋内平面図を見下ろしたときの横方向をX、縦方向をY、高さ方向をZと定義する。<br></br>
                        ・ 座標原点（0,0,0）は基準にしやすい屋内の任意の位置とする。座標系の観点から図面上一番左下の床面位置を基準にするとよい。<br></br>
                    ...
                    <p className="font-ud text-gray-700">
                        IoTカメラについてはパナソニック製の「Vieureka」を利用している。高性能なCPUが内蔵されたカメラであり、ACアダプタまたはLANケーブルを用いたPoE給電により起動する。実証実験が始まった当初は、スマートフォンを自動搬送車に設置して自己位置推定を行う案もあったが、比較を行いVieurekaを用いた自己位置推定を行うこととなった。搬送車への取り付け時には、走行時の振動の影響を受けないような設置を工夫した。
                    </p>
                    <p className="mt-4 mb-2 font-noto text-gray-900 text-base">
                    ...
                    <p className="font-ud text-gray-700">
                        ...
                        Vieurekaの場合、アップデート作業はすべて遠隔から実施出来る。障害通知も自動で行われ、軽微な障害は自動で復旧される。スマートフォンでは頻繁に発生するOSアップデートの都度、動作検証や作業員による手動点検が必要となる。<br></br>
                        <br></br>
                        2.自動起動 <br></br>
                        Vieurekaの場合、電源投入と同時に内蔵のソフトウェアも全て自動起動される。途中でソフトウェアが停止しても自動で復旧する。スマートフォンでは、画面が消える、アプリが落ちるなどの事象発生後は作業員が手動で復旧する必要がある。 <br></br>
                        <br></br>
                    ...
                    <p className="font-ud text-gray-700">
                        ...
                        ・ 場所　東海地方　屋内搬送機で入出庫する大規模倉庫（床面積　100,000㎡規模）<br></br>
                        ...
                        <br></br>
                        <p className="font-ud text-gray-700">
                            時速12km走行時の搬送車の自己位置推定は、平均絶対誤差52mm、誤差の平均値±1mm（図2）まで精度が向上している。</p>
                        <br></br>
                        <Image
                            className="rounded"
                        ...
                        <br></br>
                        <br></br>
                        <p className="font-ud text-gray-700">さらに、自己位置推定時のデータ数は全体の90%以上を取得出来るようになった。取得したデータ数の内、約60%が±50mmの範囲に収まる結果を得られている。（図3）</p>
                        <br></br>
                        <Image
                            className="rounded"
